...
def compare_models(prompt: str) -> dict:
    ...
    # Gọi model lớn (gpt-4o)
    gpt4o_text, gpt4o_latency = call_openai(prompt)
    
    # Gọi model mini (gpt-4o-mini)
    mini_text, mini_latency = call_openai_mini(prompt)
    
    # Ước tính chi phí output của GPT-4o (Dùng công thức: 0.75 từ ≈ 1 token)
    # PRICING_PER_1K_TOKENS đã được khai báo ở đầu file
    cost = (len(gpt4o_text.split()) / 0.75) / 1000 * PRICING_PER_1K_TOKENS["gpt-4o"]["output"]
    
    # Trả về dict với 5 keys (yêu cầu đặt tên key chính xác tuyệt đối)
    return {
...
def count_tokens(text: str, model: str = OPENAI_MODEL) -> int:
    ...
    try:
        import tiktoken
        # Thử lấy bộ mã hóa của model tương ứng
        enc = tiktoken.encoding_for_model(model)
        return len(enc.encode(text))
    except Exception:
        # Fallback (dự phòng) nếu model lạ (ví dụ Llama) hoặc mất mạng
...
def estimate_cost(prompt: str, response: str, model: str = OPENAI_MODEL) -> dict:
    ...
    # 2. Lấy bảng giá (nếu model lạ thì dùng giá gpt-4o làm tham chiếu)
    pricing = PRICING_PER_1K_TOKENS.get(model, PRICING_PER_1K_TOKENS["gpt-4o"])
    
    # 3. Tính tiền (Chia 1000 vì giá tính trên 1000 token)
    input_cost = input_tokens / 1000 * pricing["input"]
...
def streaming_chatbot() -> None:
    ...
    # Khởi tạo lịch sử hội thoại rỗng
    history = []
    
    while True:
        # 1. Đọc tin nhắn từ người dùng
        user_msg = input("Bạn: ")
        
        # Thoát nếu người dùng gõ quit hoặc exit
        if user_msg.strip().lower() in ("quit", "exit"):
            break
            
        # 2. Ghép history cũ với câu hỏi mới
        messages = history + [{"role": "user", "content": user_msg}]
        
        # 3. Gọi API với tham số stream=True
        ...
        # 4. In từng chữ (chunk) một ra màn hình ngay lập tức
        print("Trợ lý: ", end="", flush=True)
        reply = ""
        for chunk in stream:
            # Lấy text, nếu chunk cuối rỗng thì trả về ""
            delta = chunk.choices[0].delta.content or ""
            print(delta, end="", flush=True)
            reply += delta
        print() # Xuống dòng khi model trả lời xong
            
        # 5. Cập nhật lịch sử (câu hỏi user + câu trả lời assistant)
        history.append({"role": "user", "content": user_msg})
        history.append({"role": "assistant", "content": reply})
        
        # 6. Chỉ giữ lại tối đa 3 lượt cuối cùng (tương đương 6 messages)
        history = history[-6:]
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt == max_retries:
                raise
            # Nếu vẫn còn lượt, tính toán thời gian chờ tăng dần và sleep
            import time
...
def run_assistant(
    ...
    # Cho phép tool tự động test gõ phím thay người dùng
    if get_input is None:
        get_input = input
        
    client = OpenAI(api_key=os.getenv("OPENAI_API_KEY"))
    
    # 1. Khởi tạo các biến theo dõi phiên chat
    history, num_turns, total_tokens, total_cost = [], 0, 0, 0.0
    
    while True:
        # Kiểm tra giới hạn số lượt chat
        if max_turns is not None and num_turns >= max_turns:
            break
        ...
        # 2. Ghép Messages = (1) Persona + (2) Lịch sử + (3) Câu hỏi mới
        messages = [{"role": "system", "content": persona}] + history + [{"role": "user", "content": user_msg}]
        
        # 3. Gọi API có bọc tính năng Retry chống lỗi server
        ...
        # 4. In từng chữ từ Stream ra màn hình
        print("Trợ lý: ", end="", flush=True)
        reply = ""
        for chunk in stream:
            delta = chunk.choices[0].delta.content or ""
        ...
        # 6. Cập nhật Lịch sử (Chỉ giữ tối đa 3 lượt)
        history.append({"role": "user", "content": user_msg})
        history.append({"role": "assistant", "content": reply})
        history = history[-6:]
        
    # 7. Trả về báo cáo thống kê cuối phiên chat
    return {
        "num_turns": num_turns,
